...
"Change the Color for both performance, so each performance have its own color. Also make the range for each performance in both period same"
"""
...
# Gabungkan hanya untuk keperluan normalisasi warna
ALL_DATA = pd.concat([HasilakhirP1, HasilakhirP2], ignore_index=True)

GLOBAL_NORM = {
    'Sharpe Ratio': plt.Normalize(
        vmin=ALL_DATA['Sharpe Ratio'].min(),
        vmax=ALL_DATA['Sharpe Ratio'].max()
    ),
    'VaRSR': plt.Normalize(
        vmin=ALL_DATA['VaRSR'].min(),
        vmax=ALL_DATA['VaRSR'].max()
    )
}

GLOBAL_CMAP = {
    'Sharpe Ratio': matplotlib.colors.LinearSegmentedColormap.from_list(
        'sharpe_cmap',
        [
            '#e8f5f2',  # very light mint
            '#b2dfdb',  # soft mint
            '#4db6ac',  # teal
            '#1e88e5',  # blue
            '#0d47a1'   # deep navy
    ...
    'VaRSR': matplotlib.colors.LinearSegmentedColormap.from_list(
        'varsr_cmap',
        [
            '#fffde7',  # pale yellow
            '#fff176',  # light yellow
...
def plot_portfolio_scatters(
    data,
    metrics=['Sharpe Ratio', 'VaRSR'],
    main_title="Scatterplot"
):
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    ...
    # Marker mapping by portfolio size
    markers = ['o', 's', '^', 'v', 'D', 'P', 'X', '*', '<', '>']
    unique_num_stocks = sorted(data['Num Stocks'].unique())
    marker_map = {n: markers[i % len(markers)] for i, n in enumerate(unique_num_stocks)}
    ...
    for i, metric in enumerate(metrics):
        ax = axes[i]
        cmap = GLOBAL_CMAP[metric]
        norm = GLOBAL_NORM[metric]

        first_scatter = None

        # --- Scatter by portfolio size ---
        for num in unique_num_stocks:
            subset = data[data['Num Stocks'] == num]

            sc = ax.scatter(
                subset['Risiko_Port'],
                subset['ER Portofolio'],
                c=subset[metric],
                cmap=cmap,
                norm=norm,
                s=35,
                marker=marker_map[num],
                alpha=0.9
            )
        ...
        # --- Optimal portfolio (by metric) ---
        max_idx = data[metric].idxmax()
        max_p = data.loc[max_idx]
        portfolio_number = data.index.get_loc(max_idx) + 1  # 1-based index
        ...
        ax.text(
            0.5, -0.40,
            f"★ Optimal Portfolio #{portfolio_number} of {total_portfolios}\n"
            f"Assets: {', '.join(max_p['Kombinasi'])}\n"
            f"{metric}: {max_p[metric]:.5f}",
            transform=ax.transAxes,
            ha='center',
            va='center',
            fontsize=9,
            bbox=dict(boxstyle='round,pad=0.35', fc='white', alpha=0.6)
        )
        ...
        ax.set_title(f"Color by {metric}")
        ax.grid(True, linestyle='--', alpha=0.4)

    # --- BLACK Portfolio Size Legend (shape only) ---
    legend_handles = [
        Line2D(
            [0], [0],
            marker=marker_map[num],
    ...
    fig.legend(
        legend_handles,
        [str(num) for num in unique_num_stocks],
        title="Portfolio Size",
        loc='upper center',
        bbox_to_anchor=(0.5, 1),
        ncol=len(unique_num_stocks),
        fancybox=True,
        shadow=True
    )
...
# Periode 1
plot_portfolio_scatters(
    HasilakhirP1,
    main_title="Portfolio's Expected Return vs. Risk (pre-Danantara Period)"
)

# Periode 2
plot_portfolio_scatters(
    HasilakhirP2,
    main_title="Portfolio's Expected Return vs. Risk (post-Danantara Period)"
)